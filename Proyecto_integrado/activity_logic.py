...
class ActivityCheck:
    def __init__(self, fm=30):
        """
        fm: frecuencia de muestreo aproximada del video/stream.
        """
        self.fm = fm
        
        # Almacenamiento posiciones (8 segundos de historia)
        self.len_pos_max = 8 * self.fm 
        self.posiciones = deque(maxlen=self.len_pos_max)
        
        # Parámetros de decisión (Ajustar según resolución de cámara)
        self.dist_inact = 1.5   # Pixeles (OJO: Esto depende de la resolución)
    ...
    def estado(self, pos):
        """
        pos: [x, y]. Si es [0,0] se asume perdido.
        """
        if (pos[0] == 0 and pos[1] == 0) or len(self.posiciones) == 0:
            return "N/A"
        
        # Comparar con la última posición válida conocida o la última agregada
