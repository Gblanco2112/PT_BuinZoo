...
def create_blank_screen(message_lines, subtext=None, color=(0,0,0)):
    """Crea una imagen negra con texto centrado."""
...
def show_temporary_message(lines, duration_ms=1):
    """Muestra un mensaje y espera un tiempo (o 1ms para refrescar UI)."""
    img = create_blank_screen(lines)
    cv2.imshow(WINDOW_NAME, img)
    cv2.waitKey(duration_ms)

def draw_invalid_key_alert(img):
    """Dibuja una alerta roja de 'Tecla No Válida' sobre la imagen."""
    h, w = img.shape[:2]
    # Dibujar rectángulo rojo semi-transparente o sólido
    box_w, box_h = 600, 100
