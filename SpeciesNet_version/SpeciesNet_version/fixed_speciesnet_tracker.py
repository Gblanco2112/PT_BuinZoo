...
"""
speciesnet_tracker.py (patched v3)
New in v3:
- Motion gating: suppress detections that have near‑zero foreground motion
  (per background subtractor) unless they are very high confidence.
- Extra hygiene filters: min area, edge-touch filter, and per-frame NMS.
- only_updated=True kept so tracks are drawn only when a detection updated
  them on the current frame.
"""
...
def _nms_xyxy(dets: List[Tuple[List[float], float]], iou_thr=0.5) -> List[int]:
    # dets: [([x1,y1,x2,y2], score), ...]  returns keep indices
    if not dets: return []
...
class DetectorSpeciesNetBatch:
    ...
    def detect_batch(self, frames: List[np.ndarray]) -> List[List[Detection]]:
        ...
        for idx_item in range(len(frames)):
            ...
            # per-frame NMS to prune overlaps
            if tmp:
...
class VideoProcessor:
    def __init__(self,
                 ...
                 motion_strong_conf: float = 0.6) -> None:
    ...
    def _hygiene_filter(self, dets: List[Detection], W: int, H: int) -> List[Detection]:
        ...
        for (x1,y1,x2,y2), conf, cls in dets:
            ...
            if area < area_min: 
                continue
            # touches edges a lot → often false positives from branches/lines
