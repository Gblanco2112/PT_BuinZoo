...
"""
speciesnet_tracker.py (patched v3 - robust FP suppression)
What changed (targeting the "blue box" ghost):
- Require tracks to be confirmed with >= draw_min_hits consecutive detections
  before we draw them (default 3). This avoids 1–2 frame false positives.
- Keep only_updated=True (draw only on frames with a real detection).
- Add IoU gating against the previous EMA box; if association jumps too far
  (IoU < iou_gate), we skip drawing for that frame to prevent flickers.
- Still uses basename mapping and robust bbox parsing from earlier patches.
"""
...
class TrackerWrapper:
    def __init__(self, max_age: int = 15, n_init: int = 3, ema_alpha: float = 0.35,
                 only_updated: bool = True, draw_min_hits: int = 3, iou_gate: float = 0.2) -> None:
        ...
        self.draw_min_hits = int(draw_min_hits)
    ...
    def update(self, detections: List[Detection], frame: np.ndarray) -> List[TrackDict]:
        ...
        for tr in tracks_ds:
            ...
            if self.only_updated and getattr(tr, "time_since_update", 1) != 0:
                continue
            # need enough consecutive hits to draw
            if getattr(tr, "hits", 0) < self.draw_min_hits:
                continue
            ...
            iou_ok = _iou_xyxy(prev, new_bbox) >= self.iou_gate if prev is not None else True
            # Update EMA regardless, but draw only if iou_ok
